...
* Проверить и исправить все TODO
* Сделать валидацию всех входящих ДТО через Спринг Валидейшн
* Написать тесты на (ДАО, сервисные методы). Интеграционные тесты отдельно писать будем
* Я хочу менять размер выплат по кредиту после досрочных погашений
* БЛЯТЬ, опять сломалось перемещение по месяцам (приоритет желаний). Надо починить и прикрутить наконец тесты на это гавно!
* Большой РЕФАКТОРИНГ!
...
* Новая бизнес-идея (скорее всего будем делать отдельным сервисом) - сервис планирования путешествий
...
* надо сделать проверку на уникальность добавляемых желаний
...
* Разные списки
* Файловое облако
* Пакуемся в Докер и едем в Кубер
