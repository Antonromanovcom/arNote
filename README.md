...
* По TRCN (Трансконтейнеру) возвращаются нули - надо если так, то поискать данные по другим АПИ.
* Вынесли группировку в отдельную страницу - надо чтобы там все проверки работали. Например, если нет групп или еще что-то - чтобы это как-то отображалос, может страницу блочить вообшще как-то
