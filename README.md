...
# Что сделали:
* Исправил баг при запросе цены бумаги по тикеру и дате
...
* Поиск инструмент - однозначно можно сделать CompletableFuture и распараллеливать
* Группы
* При неправильном пароле в респонсе какой-то невразумительный ответ, а не 503-я с нормальный респонсом типа вы не авторизованы
* При коннекте из сети банка если отваливается коннект в АльфаАдвантедж все валится, а надо чтобы просто в альфа адвантедже поиска не было и вот если там сервис уже ничего не нашел - тогда просто возвращал бы пустой массив
