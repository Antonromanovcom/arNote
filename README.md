...
# Что сделали:
* Прикрутил переключатель формулы расчета дельты на фронт и бек и сохранение настроек этого переключателя
* Подчистил и переименовал ненужные поля в Дельте (фронт + бек)
* По свечам теперь считается и Доход (за все время), в том числе и проценты по нему
* Засунул getMinimalLot в КЕШ
* Запихал свечи по MOEXу в кеш
* Вроде как починил подгрузку цены при выборе даты на фронте (но надо еще проверить)


# Максимальный приоритет:

* Я выбираю дату покупки при добавлении новой бумаги. А если я выбрал выходной или праздник или торгов не было? Нужно подумать, что делать с этим кейсом?
* Нужно по запросу качать в БД праздники и выходные за определенный год, потом при попытке добавления новой бумаги запрашивать возможность добавления и если дата, которая выбрана - это праздник или выходной или не рабочий день - блочить возможность добавления бумаги.
